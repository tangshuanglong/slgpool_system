...
class MyToken
{
     ...
     * @return string
     */
    public function generateToken(string $invite_id, string $account, string $client_type, string $device_id = '')
    {
        $time = time();
        $prefix = 'app_';
        if($client_type === 'web'){
        ...
        $this->myredis->hset($this->redis_token_key, $key, $token);
        return $token;
    }
    ...
    public function checkToken(string $userToken, string $client_type, string $device_id)
    {
        ...
        list($aes_header, $base64_payload, $verify_sign) = explode('.', $userToken);
        //生成签名
        $sign = base64_encode(hash_hmac('sha256', $aes_header.'.'.$base64_payload, $this->hmac_key, true));
        //签名不一致返回登录过期
        ...
        $redis_token = $this->myredis->hget($this->redis_token_key, $key);
        ...
        if($device_id !== $header['did'] || $payload['exp'] < time()){
            ...
            $this->myredis->hdel($this->redis_token_key, $key);
            return false;
        }
